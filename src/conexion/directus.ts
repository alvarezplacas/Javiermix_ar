...
/* ==========================================================================
   SECCIÓN: FUNCIONES DE DATOS (IDs FIJOS)
   ========================================================================== */
...
export async function getHomeFiles() {
    try {
        ...
        // 🚀 Corregido para Directus v11: Usamos readFiles() en lugar de readItems()
...
export async function getLaboratorioFiles() {
    try {
        ...
        // 🚀 Corregido para Directus v11: Usamos readFiles()
        return await client.request(readFiles({
...
export async function getSerieDetails(folderId: string) {
    try {
        ...
        const items: any = await client.request(readFiles({
...
export async function addLike(artworkId: string, ip: string) { try { const client = await DirectusManager.getClient(); const file: any = await client.request(readItem('directus_files' as any, artworkId)); if (!file) return { success: false }; const filename = file.filename_download; const redisKey = `likes:${filename}`; const currentLikes = await REDIS.get(redisKey); const newLikes = (currentLikes ? parseInt(currentLikes) : 0) + 1; await REDIS.set(redisKey, newLikes.toString()); return { success: true, likes: newLikes }; } catch (e) { return { success: false }; } }
export async function getArtworkLikes(fileId: string) { try { const client = await DirectusManager.getClient(); const file: any = await client.request(readItem('directus_files' as any, fileId)); const likes = await REDIS.get(`likes:${file.filename_download}`); return likes ? parseInt(likes) : 0; } catch (e) { return 0; } }
export function getAssetUrl(id: string, options: { width?: number, format?: string, quality?: number, raw?: boolean } = {}) { if (!id) return null; const { width = 1200, format = 'avif', quality = 80 } = options; return `${PUBLIC_URL}/assets/${id}?width=${width}&format=${format}&quality=${quality}`; }
